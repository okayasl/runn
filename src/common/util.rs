...
pub fn print_matrices_comparison(input: &DenseMatrix, target: &DenseMatrix, prediction: &DenseMatrix) {
    ...
    if r == 1 {
        ...
        for j in 0..input.cols() {
            write!(input_str, "{:>7.2} ", input.at(0, j)).unwrap();
        }
        input_str.push(']');

        target_str.push_str("  [");
        for j in 0..tc {
            write!(target_str, "{:>7.2} ", target.at(0, j)).unwrap();
        }
        target_str.push(']');
        ...
        for j in 0..pc {
            write!(prediction_str, "{:>7.2} ", prediction.at(0, j)).unwrap();
        }
        prediction_str.push(']');
    ...
    for i in 1..r - 1 {
        ...
        input_str.push('⎢');
        for j in 0..input.cols() {
            write!(input_str, "{:>7.2} ", input.at(i, j)).unwrap();
        }
        input_str.push('⎥');

        target_str.push_str("  ⎢");
        for j in 0..tc {
            write!(target_str, "{:>7.2} ", target.at(i, j)).unwrap();
        }
        target_str.push('⎥');
        ...
        prediction_str.push_str(" ⎢");
        for j in 0..pc {
            write!(prediction_str, "{:>7.2} ", prediction.at(i, j)).unwrap();
        }
        prediction_str.push('⎥');
    ...
    input_str.push('⎣');
    for j in 0..input.cols() {
        write!(input_str, "{:>7.2} ", input.at(r - 1, j)).unwrap();
    }
    input_str.push('⎦');

    target_str.push_str("  ⎣");
    for j in 0..tc {
        write!(target_str, "{:>7.2} ", target.at(r - 1, j)).unwrap();
    }
    target_str.push('⎦');
    ...
    prediction_str.push_str(" ⎣");
    for j in 0..pc {
        write!(prediction_str, "{:>7.2} ", prediction.at(r - 1, j)).unwrap();
    }
    prediction_str.push('⎦');
